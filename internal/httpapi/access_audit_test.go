...
func TestAccessAuditRecordsSafeAuthenticationEvent(t *testing.T) {
	...
	if len(recorder.events) != 1 { t.Fatalf("expected one audit event, got %d", len(recorder.events)) }
	event := recorder.events[0]
	if event.Action != "auth.login.success" || event.ActorID != "usr_123" || event.EntityID != "usr_123" { t.Fatalf("unexpected audit event: %#v", event) }
	if event.RequestID != "req_test" { t.Fatalf("expected request id req_test, got %q", event.RequestID) }
	if len(event.Metadata) != 1 || event.Metadata["outcome"] != "accepted" { t.Fatalf("unexpected metadata: %#v", event.Metadata) }
}
...
func TestAccessAuditNeverStoresAuthenticationSecrets(t *testing.T) {
	...
	event := recorder.events[0]
	if _, ok := event.Metadata["password"]; ok { t.Fatal("audit metadata must not include passwords") }
	if _, ok := event.Metadata["token"]; ok { t.Fatal("audit metadata must not include session tokens") }
	if event.RequestID != "req_secret" { t.Fatalf("expected request id req_secret, got %q", event.RequestID) }
}
