...
func TestNormalizeMovementHistoryWindow(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			if got := normalizeMovementHistoryWindow(tc.in); got != tc.want { t.Fatalf("got %d, want %d", got, tc.want) }
		})
	}
}
...
func TestNormalizeMovementHistoryLimit(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			if got := normalizeMovementHistoryLimit(tc.in); got != tc.want { t.Fatalf("got %d, want %d", got, tc.want) }
		})
	}
}
