...
func (s *Store) CreateWarehouse(ctx context.Context, warehouse domain.Warehouse) error {
	...
	_, err := s.pool.Exec(ctx, `
		INSERT INTO warehouses (id, code, name, address, timezone, active)
		VALUES ($1, $2, $3, $4, $5, $6)`,
		warehouse.ID, strings.TrimSpace(warehouse.Code), strings.TrimSpace(warehouse.Name), strings.TrimSpace(warehouse.Address), strings.TrimSpace(warehouse.Timezone), warehouse.Active)
	return mapError(err)
}
...
func (s *Store) CreateLocation(ctx context.Context, location domain.Location) error {
	...
	_, err := s.pool.Exec(ctx, `
		INSERT INTO locations (id, warehouse_id, code, name, description, active)
		VALUES ($1, $2, $3, $4, $5, $6)`,
		location.ID, location.WarehouseID, strings.TrimSpace(location.Code), strings.TrimSpace(location.Name), strings.TrimSpace(location.Description), location.Active)
	return mapError(err)
}
...
func (s *Store) CreateLot(ctx context.Context, lot domain.Lot) error {
	...
	_, err := s.pool.Exec(ctx, `
		...
		VALUES ($1, $2, $3, $4, $5)`, lot.ID, lot.ProductID, strings.TrimSpace(lot.LotNumber), lot.Manufactured, lot.ExpiresAt)
	return mapError(err)
}
...
func (s *Store) Transfer(ctx context.Context, request domain.TransferRequest) error {
	...
	keys := []string{
		balanceLockKey(request.ProductID, request.FromLocationID, request.LotID),
		balanceLockKey(request.ProductID, request.ToLocationID, request.LotID),
	}
	sort.Strings(keys)
	...
	out := domain.StockMovement{
		ID: request.ID + "_out", ProductID: request.ProductID, LocationID: request.FromLocationID, LotID: request.LotID,
		Type: domain.MovementTransferOut, QuantityDelta: -request.Quantity, Reference: reference, Note: request.Note, ActorID: request.ActorID, OccurredAt: request.OccurredAt,
	}
	if _, err := applyMovementTx(ctx, tx, out); err != nil {
		return err
	}
	in := domain.StockMovement{
		ID: request.ID + "_in", ProductID: request.ProductID, LocationID: request.ToLocationID, LotID: request.LotID,
		Type: domain.MovementTransferIn, QuantityDelta: request.Quantity, Reference: reference, Note: request.Note, ActorID: request.ActorID, OccurredAt: request.OccurredAt,
	}
...
func (s *Store) ListLowStock(ctx context.Context, limit int) ([]domain.StockBalance, error) {
	...
	rows, err := s.pool.Query(ctx, `
		SELECT b.product_id, b.location_id, COALESCE(b.lot_id, ''), b.quantity, b.updated_at
...
func applyMovementTx(ctx context.Context, tx pgx.Tx, movement domain.StockMovement) (domain.StockBalance, error) {
	...
	if errors.Is(err, pgx.ErrNoRows) {
		...
		if err := tx.QueryRow(ctx, `
			INSERT INTO inventory_balances (id, product_id, location_id, lot_id, quantity)
			VALUES ($1, $2, $3, NULLIF($4, ''), $5)
			RETURNING updated_at`, balanceID, movement.ProductID, movement.LocationID, movement.LotID, newQuantity).Scan(&updatedAt); err != nil {
			return domain.StockBalance{}, mapError(err)
		}
	} else {
	...
	_, err = tx.Exec(ctx, `
		INSERT INTO stock_movements (id, product_id, location_id, lot_id, movement_type, quantity_delta, reference, note, actor_id, occurred_at)
		VALUES ($1, $2, $3, NULLIF($4, ''), $5, $6, $7, $8, $9, $10)`,
		movement.ID, movement.ProductID, movement.LocationID, movement.LotID, movement.Type, movement.QuantityDelta,
		strings.TrimSpace(movement.Reference), strings.TrimSpace(movement.Note), strings.TrimSpace(movement.ActorID), movement.OccurredAt)
	if err != nil {
		return domain.StockBalance{}, mapError(err)
	}
