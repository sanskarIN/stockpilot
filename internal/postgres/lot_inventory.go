...
func (s *Store) ListLotInventory(ctx context.Context, filter repository.LotInventoryFilter) ([]domain.LotInventoryRow, error) {
	...
	if strings.TrimSpace(filter.ProductID) != "" {
		conditions = append(conditions, "b.product_id="+arg(strings.TrimSpace(filter.ProductID)))
	}
	if strings.TrimSpace(filter.WarehouseID) != "" {
		conditions = append(conditions, "w.id="+arg(strings.TrimSpace(filter.WarehouseID)))
	}
	if strings.TrimSpace(filter.LocationID) != "" {
		conditions = append(conditions, "loc.id="+arg(strings.TrimSpace(filter.LocationID)))
	}
	if strings.TrimSpace(filter.LotID) != "" {
		conditions = append(conditions, "l.id="+arg(strings.TrimSpace(filter.LotID)))
	}
	if filter.ExpiringBy != nil {
		conditions = append(conditions, "l.expires_at IS NOT NULL", "l.expires_at <= "+arg(*filter.ExpiringBy))
	}
...
LIMIT ` + limitArg + ` OFFSET ` + offsetArg
	...
	for rows.Next() {
		...
		if err := rows.Scan(&item.ProductID, &item.SKU, &item.ProductName, &item.LotID, &item.LotNumber, &item.LocationID, &item.Location, &item.WarehouseID, &item.Warehouse, &item.OnHand, &item.ExpiresAt, &item.Active); err != nil {
			return nil, err
		}
