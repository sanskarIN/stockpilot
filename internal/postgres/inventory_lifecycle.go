...
func (s *Store) UpdateLocation(ctx context.Context, item domain.Location) error {
	...
	var active bool
	if err := tx.QueryRow(ctx, `SELECT warehouse_id, active FROM locations WHERE id=$1 FOR UPDATE`, item.ID).Scan(&currentWarehouse, &active); err != nil {
		return mapError(err)
	}
	if !item.Active && active {
		var onHand int64
		if err := tx.QueryRow(ctx, `SELECT COALESCE(sum(quantity),0) FROM inventory_balances WHERE location_id=$1`, item.ID).Scan(&onHand); err != nil {
			return err
		}
		if onHand != 0 {
			return fmt.Errorf("%w: location has non-zero inventory", domain.ErrConflict)
		}
	}
	var warehouseActive bool
	if err := tx.QueryRow(ctx, `SELECT active FROM warehouses WHERE id=$1`, item.WarehouseID).Scan(&warehouseActive); err != nil {
		return mapError(err)
	}
	if !warehouseActive && item.Active {
		return fmt.Errorf("%w: location cannot be active in an archived warehouse", domain.ErrConflict)
	}
	if _, err := tx.Exec(ctx, `UPDATE locations SET warehouse_id=$2,code=$3,name=$4,active=$5 WHERE id=$1`, item.ID, item.WarehouseID, strings.TrimSpace(item.Code), strings.TrimSpace(item.Name), item.Active); err != nil {
		return mapError(err)
	}
	_ = currentWarehouse
	return tx.Commit(ctx)
