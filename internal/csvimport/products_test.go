...
func TestParseProductsValidRows(t *testing.T) {
	...
	if result.Rows[0].Product.SKU != "SKU-1" || result.Rows[0].Product.UnitCostMinor != 1250 {
		t.Fatalf("unexpected product: %+v", result.Rows[0].Product)
	}
}
...
func TestParseProductsReportsFieldErrors(t *testing.T) {
	...
	if len(result.Errors) < 2 {
		t.Fatalf("errors = %+v, want multiple validation errors", result.Errors)
	}
}
...
func TestParseProductsRejectsDuplicateSKUAndBarcode(t *testing.T) {
	...
	if len(result.Errors) != 2 {
		t.Fatalf("errors = %+v, want duplicate SKU and barcode", result.Errors)
	}
}
