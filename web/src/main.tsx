...
function App() {
  ...
  useEffect(() => { let active = true; void stockpilotAPI.me().then((user) => { if (active) setSession({ kind: "signed-in", user }); }, () => { if (active) setSession({ kind: "signed-out" }); }); return () => { active = false; }; }, []);
