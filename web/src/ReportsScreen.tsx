...
import { ReplenishmentReadinessPanel } from "./ReplenishmentReadinessPanel";
import { ReplenishmentReviewHistoryPanel } from "./ReplenishmentReviewHistoryPanel";
import type { InventoryValuationReport, InventorySummary, PurchasingSummary, User, InventoryAgingReport, StockMovementHistoryReport, SupplierPerformanceReport } from "./types";
...
const emptySupplierPerformance: SupplierPerformanceReport = { asOf: "", windowDays: 30, items: [] };
...
export function ReportsScreen({ user, onBack, onSessionExpired }: Props) {
  ...
  const [supplierPerformance, setSupplierPerformance] = useState<SupplierPerformanceReport>(emptySupplierPerformance);
  ...
  const load = useCallback(async () => {
    ...
    try {
      const [overview, valuationReport, agingReport, movementReport, supplierReport] = await Promise.all([
        stockpilotAPI.reportOverview(), stockpilotAPI.inventoryValuation(), inventoryAging(500), stockMovementHistory(30, 500), stockpilotAPI.requestSupplierPerformance(30, 500),
      ]);
      setInventory(overview.inventory); setPurchasing(overview.purchasing); setValuation(valuationReport); setAging(agingReport); setMovementHistory(movementReport); setSupplierPerformance(supplierReport); setState({ kind: "ready" });
    } catch (error) {
  ...
  function exportSupplierPerformance() { window.location.assign(`/api/v1/reports/supplier-performance?format=csv&days=${supplierPerformance.windowDays}&limit=5000`); }

  return <div className="app-shell"><main className="content report-page" tabIndex={-1}>
    <header className="topbar"><div><p className="eyebrow">Operational reporting</p><h1>Reports & analytics</h1><p className="muted">Read-only reporting built from authoritative catalog, inventory, purchasing, valuation, movement, supplier, and replenishment data.</p></div><div className="topbar-actions"><button className="secondary-button" type="button" onClick={onBack}>Back to dashboard</button><button className="primary-button" type="button" onClick={() => void load()} disabled={state.kind === "loading"}>Refresh</button></div></header>
    ...
    <section className="dashboard-grid report-grid">
      ...
      <article className="panel wide"><div className="panel-heading"><div><p className="eyebrow">Supplier performance</p><h2>Purchasing reliability</h2></div>{canExport && <button className="secondary-button compact-button" type="button" onClick={exportSupplierPerformance}>Export CSV</button>}</div><p className="muted">Orders created in the last {supplierPerformance.windowDays} days, with observed receipt lead time and on-time completion.</p><div className="table-wrap"><table><thead><tr><th>Supplier</th><th className="numeric">Orders</th><th className="numeric">Open units</th><th className="numeric">Lead time</th><th className="numeric">On time</th></tr></thead><tbody>{supplierPerformance.items.slice(0, 25).map(item => <tr key={item.supplierId}><td><strong>{item.supplierName}</strong><br/><span className="compact mono">{item.supplierCode}</span></td><td className="numeric">{item.orderCount.toLocaleString()}</td><td className="numeric">{item.openUnits.toLocaleString()}</td><td className="numeric">{item.completedOrderCount ? `${item.averageLeadTimeDays.toFixed(1)}d` : "—"}</td><td className="numeric">{item.completedOrderCount ? `${item.onTimeOrderCount.toLocaleString()}/${item.completedOrderCount.toLocaleString()}` : "—"}</td></tr>)}{supplierPerformance.items.length === 0 && state.kind !== "loading" && <tr><td colSpan={5}>No supplier activity in the selected window.</td></tr>}</tbody></table></div></article>
      ...
      <ReplenishmentReadinessPanel canExport={canExport} onSessionExpired={onSessionExpired} />
      <ReplenishmentReviewHistoryPanel onSessionExpired={onSessionExpired} />
    </section>
  </main></div>;
}
