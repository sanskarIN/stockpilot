...
import type { InventoryValuationReport, InventorySummary, PurchasingSummary, User } from "./types";
...
export function ReportsScreen({ user, onBack, onSessionExpired }: Props) {
  ...
  const load = useCallback(async () => {
    ...
    try {
      const [overview, valuationReport] = await Promise.all([
        stockpilotAPI.reportOverview(),
        stockpilotAPI.inventoryValuation(),
      ]);
      setInventory(overview.inventory);
      setPurchasing(overview.purchasing);
  ...
  function exportValuation() {
    window.location.assign("/api/v1/reports/inventory-valuation/export.csv?limit=5000");
  }

  return <div className="app-shell"><main className="content report-page" tabIndex={-1}>
    <header className="topbar"><div><p className="eyebrow">Operational reporting</p><h1>Reports & analytics</h1><p className="muted">Read-only reporting built from the authoritative catalog, inventory, purchasing, and valuation data.</p></div><div className="topbar-actions"><button className="secondary-button" type="button" onClick={onBack}>Back to dashboard</button><button className="primary-button" type="button" onClick={() => void load()} disabled={state.kind === "loading"}>Refresh</button></div></header>
